...
* [next](codecs.html "codecs — Codec registry and base classes") |
* [previous](binary.html "Binary Data Services") |
...
# `struct` — Interpret bytes as packed binary data[¶](#struct-interpret-bytes-as-packed-binary-data "Link to this heading")
...
as Python [`bytes`](stdtypes.html#bytes "bytes") objects. Compact [format strings](#struct-format-strings)
...
Several `struct` functions (and methods of [`Struct`](#struct.Struct "struct.Struct")) take a *buffer*
...
for that purpose are [`bytes`](stdtypes.html#bytes "bytes") and [`bytearray`](stdtypes.html#bytearray "bytearray"), but many other types
that can be viewed as an array of bytes implement the buffer protocol, so that
they can be read/filled without additional copying from a [`bytes`](stdtypes.html#bytes "bytes") object.

## Functions and Exceptions[¶](#functions-and-exceptions "Link to this heading")

The module defines the following exception and functions:

*exception* struct.error[¶](#struct.error "Link to this definition")
:   Exception raised on various occasions; argument is a string describing what
    is wrong.

struct.pack(*format*, *v1*, *v2*, *...*)[¶](#struct.pack "Link to this definition")
:   Return a bytes object containing the values *v1*, *v2*, … packed according
    to the format string *format*. The arguments must match the values required by
    the format exactly.

struct.pack\_into(*format*, *buffer*, *offset*, *v1*, *v2*, *...*)[¶](#struct.pack_into "Link to this definition")
:   Pack the values *v1*, *v2*, … according to the format string *format* and
    write the packed bytes into the writable buffer *buffer* starting at
    position *offset*. Note that *offset* is a required argument.

struct.unpack(*format*, *buffer*)[¶](#struct.unpack "Link to this definition")
:   Unpack from the buffer *buffer* (presumably packed by `pack(format, ...)`)
    according to the format string *format*. The result is a tuple even if it
    contains exactly one item. The buffer’s size in bytes must match the
    size required by the format, as reflected by [`calcsize()`](#struct.calcsize "struct.calcsize").

struct.unpack\_from(*format*, */*, *buffer*, *offset=0*)[¶](#struct.unpack_from "Link to this definition")
:   Unpack from *buffer* starting at position *offset*, according to the format
    string *format*. The result is a tuple even if it contains exactly one
    item. The buffer’s size in bytes, starting at position *offset*, must be at
    least the size required by the format, as reflected by [`calcsize()`](#struct.calcsize "struct.calcsize").

struct.iter\_unpack(*format*, *buffer*)[¶](#struct.iter_unpack "Link to this definition")
:   Iteratively unpack from the buffer *buffer* according to the format
    string *format*. This function returns an iterator which will read
    equally sized chunks from the buffer until all its contents have been
    consumed. The buffer’s size in bytes must be a multiple of the size
    required by the format, as reflected by [`calcsize()`](#struct.calcsize "struct.calcsize").

    Each iteration yields a tuple as specified by the format string.

    Added in version 3.4.

struct.calcsize(*format*)[¶](#struct.calcsize "Link to this definition")
:   Return the size of the struct (and hence of the bytes object produced by
    `pack(format, ...)`) corresponding to the format string *format*.

## Format Strings[¶](#format-strings "Link to this heading")
...
### Byte Order, Size, and Alignment[¶](#byte-order-size-and-alignment "Link to this heading")
...
### Format Characters[¶](#format-characters "Link to this heading")
...
raised [`DeprecationWarning`](exceptions.html#DeprecationWarning "DeprecationWarning") instead of [`struct.error`](#struct.error "struct.error").
...
### Native Formats[¶](#native-formats "Link to this heading")
...
### Standard Formats[¶](#standard-formats "Link to this heading")
...
*class* struct.Struct(*format*)[¶](#struct.Struct "Link to this definition")
:   Return a new Struct object which writes and reads binary data according to
    ...
    format strings needn’t worry about reusing a single [`Struct`](#struct.Struct "struct.Struct")
    instance.

    Compiled Struct objects support the following methods and attributes:

    pack(*v1*, *v2*, *...*)[¶](#struct.Struct.pack "Link to this definition")
    :   Identical to the [`pack()`](#struct.pack "struct.pack") function, using the compiled format.
        (`len(result)` will equal [`size`](#struct.Struct.size "struct.Struct.size").)

    pack\_into(*buffer*, *offset*, *v1*, *v2*, *...*)[¶](#struct.Struct.pack_into "Link to this definition")
    :   Identical to the [`pack_into()`](#struct.pack_into "struct.pack_into") function, using the compiled format.

    unpack(*buffer*)[¶](#struct.Struct.unpack "Link to this definition")
    :   Identical to the [`unpack()`](#struct.unpack "struct.unpack") function, using the compiled format.
        The buffer’s size in bytes must equal [`size`](#struct.Struct.size "struct.Struct.size").

    unpack\_from(*buffer*, *offset=0*)[¶](#struct.Struct.unpack_from "Link to this definition")
    :   Identical to the [`unpack_from()`](#struct.unpack_from "struct.unpack_from") function, using the compiled format.
        The buffer’s size in bytes, starting at position *offset*, must be at least
        [`size`](#struct.Struct.size "struct.Struct.size").

    iter\_unpack(*buffer*)[¶](#struct.Struct.iter_unpack "Link to this definition")
    :   Identical to the [`iter_unpack()`](#struct.iter_unpack "struct.iter_unpack") function, using the compiled format.
        The buffer’s size in bytes must be a multiple of [`size`](#struct.Struct.size "struct.Struct.size").

        Added in version 3.4.

    format[¶](#struct.Struct.format "Link to this definition")
    :   The format string used to construct this Struct object.

        Changed in version 3.7: The format string type is now [`str`](stdtypes.html#str "str") instead of [`bytes`](stdtypes.html#bytes "bytes").

    size[¶](#struct.Struct.size "Link to this definition")
    :   The calculated size of the struct (and hence of the bytes object produced
        by the [`pack()`](#struct.pack "struct.pack") method) corresponding to [`format`](functions.html#format "format").
...
[Binary Data Services](binary.html "previous chapter")

#### Next topic

[`codecs` — Codec registry and base classes](codecs.html "next chapter")
