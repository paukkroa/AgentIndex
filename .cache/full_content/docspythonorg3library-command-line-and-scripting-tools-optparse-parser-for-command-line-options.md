...
* [next](getpass.html "getpass — Portable password input") |
* [previous](../howto/argparse-optparse.html "Migrating optparse code to argparse") |
...
# `optparse` — Parser for command line options[¶](#module-optparse "Link to this heading")

**Source code:** [Lib/optparse.py](https://github.com/python/cpython/tree/3.14/Lib/optparse.py)

---

## Choosing an argument parsing library[¶](#choosing-an-argument-parsing-library "Link to this heading")
...
`optparse` should be considered as an alternative to [`argparse`](argparse.html#module-argparse "argparse: Command-line option and argument parsing library.") in the
following cases:
...
### What are options for?[¶](#what-are-options-for "Link to this heading")
...
### What are positional arguments for?[¶](#what-are-positional-arguments-for "Link to this heading")
...
### Understanding option actions[¶](#understanding-option-actions "Link to this heading")
...
### The store action[¶](#the-store-action "Link to this heading")
...
### Handling boolean (flag) options[¶](#handling-boolean-flag-options "Link to this heading")
...
### Other actions[¶](#other-actions "Link to this heading")
...
### Generating help[¶](#generating-help "Link to this heading")
...
* options that have a default value can include `%default` in the help
  string—`optparse` will replace it with [`str()`](stdtypes.html#str "str") of the option’s
  default value. If an option has no default value (or the default value is
  `None`), `%default` expands to `none`.

#### Grouping Options[¶](#grouping-options "Link to this heading")
...
*class* optparse.OptionGroup(*parser*, *title*, *description=None*)[¶](#optparse.OptionGroup "Link to this definition")
:   where
...
OptionParser.get\_option\_group(*opt\_str*)[¶](#optparse.OptionParser.get_option_group "Link to this definition")
...
### Printing a version string[¶](#printing-a-version-string "Link to this heading")
...
OptionParser.print\_version(*file=None*)[¶](#optparse.OptionParser.print_version "Link to this definition")
...
### How `optparse` handles errors[¶](#how-optparse-handles-errors "Link to this heading")
...
### Creating the parser[¶](#creating-the-parser "Link to this heading")

The first step in using `optparse` is to create an OptionParser instance.

*class* optparse.OptionParser(*...*)[¶](#optparse.OptionParser "Link to this definition")
...
### Populating the parser[¶](#populating-the-parser "Link to this heading")
...
### Defining options[¶](#defining-options "Link to this heading")
...
OptionParser.add\_option(*option*)[¶](#optparse.OptionParser.add_option "Link to this definition")
...
*class* optparse.Values[¶](#optparse.Values "Link to this definition")
...
### Option attributes[¶](#option-attributes "Link to this heading")

*class* optparse.Option[¶](#optparse.Option "Link to this definition")
...
`optparse` raises [`OptionError`](#optparse.OptionError "optparse.OptionError").
...
:   (default: derived from option strings)
    ...
    tells `optparse` where to write it: [`dest`](#optparse.Option.dest "optparse.Option.dest") names an
...
### Standard option actions[¶](#standard-option-actions "Link to this heading")
...
* `"store"` [relevant: [`type`](#optparse.Option.type "optparse.Option.type"), [`dest`](#optparse.Option.dest "optparse.Option.dest"),
  ...
  If [`dest`](#optparse.Option.dest "optparse.Option.dest") is not supplied, `optparse` derives a destination
...
* `"append_const"` [required: [`const`](#optparse.Option.const "optparse.Option.const"); relevant:
  ...
  Like `"store_const"`, but the value [`const`](#optparse.Option.const "optparse.Option.const") is appended to
...
* `"help"`
  ...
  `optparse` automatically adds a [`help`](#optparse.Option.help "optparse.Option.help") option to all
...
### Standard option types[¶](#standard-option-types "Link to this heading")
...
### Parsing arguments[¶](#parsing-arguments "Link to this heading")

The whole point of creating and populating an OptionParser is to call its
[`parse_args()`](#optparse.OptionParser.parse_args "optparse.OptionParser.parse_args") method.

OptionParser.parse\_args(*args=None*, *values=None*)[¶](#optparse.OptionParser.parse_args "Link to this definition")
...
### Querying and manipulating your option parser[¶](#querying-and-manipulating-your-option-parser "Link to this heading")
...
OptionParser.get\_option(*opt\_str*)[¶](#optparse.OptionParser.get_option "Link to this definition")
:   Returns the Option instance with the option string *opt\_str*, or `None` if
    no options have that option string.

OptionParser.has\_option(*opt\_str*)[¶](#optparse.OptionParser.has_option "Link to this definition")
:   Return `True` if the OptionParser has an option with option string *opt\_str*
    (e.g., `-q` or `--verbose`).

OptionParser.remove\_option(*opt\_str*)[¶](#optparse.OptionParser.remove_option "Link to this definition")
...
### Conflicts between options[¶](#conflicts-between-options "Link to this heading")
...
As an example, let’s define an [`OptionParser`](#optparse.OptionParser "optparse.OptionParser") that resolves conflicts
...
OptionParser.set\_usage(*usage*)[¶](#optparse.OptionParser.set_usage "Link to this definition")
...
OptionParser.print\_usage(*file=None*)[¶](#optparse.OptionParser.print_usage "Link to this definition")
...
OptionParser.set\_defaults(*dest=value*, *...*)[¶](#optparse.OptionParser.set_defaults "Link to this definition")
...
## Option Callbacks[¶](#option-callbacks "Link to this heading")
...
### Defining a callback option[¶](#defining-a-callback-option "Link to this heading")
...
### How callbacks are called[¶](#how-callbacks-are-called "Link to this heading")
...
### Raising errors in a callback[¶](#raising-errors-in-a-callback "Link to this heading")
...
### Callback example 1: trivial callback[¶](#callback-example-1-trivial-callback "Link to this heading")
...
### Callback example 2: check option order[¶](#callback-example-2-check-option-order "Link to this heading")
...
### Callback example 3: check option order (generalized)[¶](#callback-example-3-check-option-order-generalized "Link to this heading")
...
### Callback example 4: check arbitrary condition[¶](#callback-example-4-check-arbitrary-condition "Link to this heading")
...
### Callback example 5: fixed arguments[¶](#callback-example-5-fixed-arguments "Link to this heading")
...
### Callback example 6: variable arguments[¶](#callback-example-6-variable-arguments "Link to this heading")
...
## Extending `optparse`[¶](#extending-optparse "Link to this heading")

Since the two major controlling factors in how `optparse` interprets
command-line options are the action and type of each option, the most likely
direction of extension is to add new actions and new types.

### Adding new types[¶](#adding-new-types "Link to this heading")
...
`optparse`’s types: [`TYPES`](#optparse.Option.TYPES "optparse.Option.TYPES") and [`TYPE_CHECKER`](#optparse.Option.TYPE_CHECKER "optparse.Option.TYPE_CHECKER").
...
up modifying the [`TYPE_CHECKER`](#optparse.Option.TYPE_CHECKER "optparse.Option.TYPE_CHECKER") attribute of `optparse`’s
...
### Adding new actions[¶](#adding-new-actions "Link to this heading")
...
*exception* optparse.OptionError[¶](#optparse.OptionError "Link to this definition")
:   Raised if an [`Option`](#optparse.Option "optparse.Option") instance is created with invalid or
    inconsistent arguments.

*exception* optparse.OptionConflictError[¶](#optparse.OptionConflictError "Link to this definition")
:   Raised if conflicting options are added to an [`OptionParser`](#optparse.OptionParser "optparse.OptionParser").

*exception* optparse.OptionValueError[¶](#optparse.OptionValueError "Link to this definition")
:   Raised if an invalid option value is encountered on the command line.

*exception* optparse.BadOptionError[¶](#optparse.BadOptionError "Link to this definition")
:   Raised if an invalid option is passed on the command line.

*exception* optparse.AmbiguousOptionError[¶](#optparse.AmbiguousOptionError "Link to this definition")
:   Raised if an ambiguous option is passed on the command line.

### [Table of Contents](../contents.html)

* [`optparse` — Parser for command line options](#)
  + [Choosing an argument parsing library](#choosing-an-argument-parsing-library)
  + [Introduction](#introduction)
  + [Background](#background)
    - [Terminology](#terminology)
    - [What are options for?](#what-are-options-for)
    - [What are positional arguments for?](#what-are-positional-arguments-for)
  + [Tutorial](#tutorial)
    - [Understanding option actions](#understanding-option-actions)
    - [The store action](#the-store-action)
    ...
    - [How `optparse` handles errors](#how-optparse-handles-errors)
    - [Putting it all together](#putting-it-all-together)
  + [Reference Guide](#reference-guide)
    - [Creating the parser](#creating-the-parser)
    - [Populating the parser](#populating-the-parser)
    - [Defining options](#defining-options)
    - [Option attributes](#option-attributes)
    - [Standard option actions](#standard-option-actions)
    ...
    - [Conflicts between options](#conflicts-between-options)
    - [Cleanup](#cleanup)
    - [Other methods](#other-methods)
  + [Option Callbacks](#option-callbacks)
    - [Defining a callback option](#defining-a-callback-option)
    - [How callbacks are called](#how-callbacks-are-called)
  ...
  + [Extending `optparse`](#extending-optparse)
...
[Migrating `optparse` code to `argparse`](../howto/argparse-optparse.html "previous chapter")

#### Next topic

[`getpass` — Portable password input](getpass.html "next chapter")
